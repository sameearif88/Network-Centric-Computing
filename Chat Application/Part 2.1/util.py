...
def get_key(clientData, address):
    '''
    This function returns the corrosponding key by pass the value 
    '''
...
def make_chunks(message):
    chunks = []
    start = 0
    for i in range(0, len(message)):
        if len(message[start:i]) == CHUNK_SIZE:
            chunks.append(message[start:i])
            start = i
        if i == len(message) - 1:
